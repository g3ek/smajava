...
public class Engine {
    ...
    public void process(Inverter inverter) throws IOException, SmajavaException, InterruptedException {        
        ...
        while ((line = bufferedReader.readLine()) != null) {
            linecntr++;
            if (line.startsWith("S")) { // sending
                send(line);
            } else if (line.startsWith("R")) { // receiving
                received = receive(line);
            } else if (line.startsWith("E")) { // extracting
                try {
                    ...
                    if (e.getType() == SmajavaException.Type.BLUETOOTH_RESET) {
                        ...
                        // go to the last marked position and re-read lines from there
    ...
    public byte[] receive(String line) throws IOException, SmajavaException {
        ...
        while (!found) {
            ...
            // compare received data with expected generated by encoder
            // we're not interested in the first 4 header bytes 
